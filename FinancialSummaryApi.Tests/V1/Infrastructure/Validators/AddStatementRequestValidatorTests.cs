...
namespace FinancialSummaryApi.Tests.V1.Infrastructure.Validators
{
    public class AddStatementRequestValidatorTests
    {
        ...
        [InlineData(-2)]
        [InlineData(-1)]
        public void GivenInvalidChargedAmount_ShouldHaveValidationError(int chargedAmount)
        {
            var statement = new AddStatementRequest()
            {
                TargetId = new Guid("58daf21a-e2d5-475f-87f4-1c0c7f1ffb10"),
            ...
            result.ShouldHaveValidationErrorFor(x => x.ChargedAmount)
                  .WithErrorMessage($"'{InsertSpaceBetweenWords(nameof(statement.ChargedAmount))}' must be greater than or equal to '0'.");
        }

        [Theory]
        [InlineData(-2)]
        [InlineData(-1)]
        public void GivenInvalidPaidAmount_ShouldHaveValidationError(int paidAmount)
        {
            var statement = new AddStatementRequest()
            {
                TargetId = new Guid("58daf21a-e2d5-475f-87f4-1c0c7f1ffb10"),
            ...
            result.ShouldHaveValidationErrorFor(x => x.PaidAmount)
                  .WithErrorMessage($"'{InsertSpaceBetweenWords(nameof(statement.PaidAmount))}' must be greater than or equal to '0'.");
        }

        [Theory]
        [InlineData(-2)]
        [InlineData(-1)]
        public void GivenInvalidHousingBenefitAmount_ShouldHaveValidationError(int housingBenefitAmount)
        {
            var statement = new AddStatementRequest()
            {
                TargetId = new Guid("58daf21a-e2d5-475f-87f4-1c0c7f1ffb10"),
            ...
            result.ShouldHaveValidationErrorFor(x => x.HousingBenefitAmount)
                  .WithErrorMessage($"'{InsertSpaceBetweenWords(nameof(statement.HousingBenefitAmount))}' must be greater than or equal to '0'.");
        }
